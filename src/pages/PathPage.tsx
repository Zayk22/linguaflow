...
export function PathPage() {
  ...
  // Only render lessons that have actual vocabulary content
  const lessonsWithContent = new Set(VOCABULARY_ITEMS.map((item) => item.sourceLessonId))
    ...
    >
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 md:px-8">
        ...
        {path.map((unit, unitIndex) => {
          const visibleLessons = unit.lessons.filter((lesson) => lessonsWithContent.has(lesson.id))
        ...
        >
          ...
          <p className="font-medium">More lessons coming soon</p>
          <p className="text-sm text-muted-foreground mt-1">
            New content is being added to expand the path
          </p>
        </motion.div>
      </div>
    </div>
